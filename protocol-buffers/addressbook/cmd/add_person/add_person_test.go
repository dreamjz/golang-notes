...
func TestPromptForAddressReturnAddress(t *testing.T) {
	in := `12345
Example Name
name@example.com
123-456-7890
...
`
	...
	if got.Id != 12345 {
		t.Errorf("promptForAddress(%q) got %d, want ID %d", in, got.Id, 12346)
	}
	...
	if got.Email != "name@example.com" {
		t.Errorf("promptForAddress(%q) => want email %q, git %q", in, "name@example.com", got.Email)
	}
